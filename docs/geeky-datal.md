...
|---|---|---|
| `geeky.id` | **Live / production** | Storefront asli yang berfungsi. Ini yang harus didaftarkan ke Midtrans, dipakai sebagai `NEXT_PUBLIC_APP_URL`, dan jadi fallback di semua kode. |
...
|---|---|---|
| `support@geeky.id` | Email kontak publik (halaman Contact, FAQ, Terms, Privacy, Refund) | Literal string di kode — **sedang diseragamkan dari `support@geekytech.com` di putaran perbaikan item #8** |
...
| Nomor | Dipakai untuk | Sumber | Status |
|---|---|---|---|
| `6281234567890` | Nomor WhatsApp CS publik (`/contact`, footer, halaman legal, FAQ) | `settings.whatsapp_cs` (tabel `settings`, diedit lewat Admin → Settings) | ⚠️ **Masih placeholder seed, belum diganti nomor asli** |
| `6281992283947` | Nomor telepon pengirim untuk label pengiriman (Biteship) | `settings.store_origin.phone` (diedit lewat Admin → Settings → Shipping) | Terlihat seperti nomor asli (bukan pola placeholder) — **perlu konfirmasi apakah ini nomor yang sama dengan CS WhatsApp**, atau memang nomor gudang/kurir yang berbeda |

**Aksi yang disarankan:** cek ke pemilik bisnis apakah `6281992283947` adalah nomor WhatsApp CS yang sebenarnya. Kalau iya, cukup salin nilainya ke setting `whatsapp_cs` lewat Admin → Settings (tidak perlu ubah kode).
...
| Field | Nilai | Sumber |
|---|---|---|
| Nama badan usaha | **CV. Sentosa Berkat Jaya** | Hardcode di `components/store/store-footer.tsx` (copyright line); akan dipindah ke `lib/constants/business-identity.ts` di putaran ini dan ditambahkan juga ke halaman About & Contact |

Belum ada data NPWP/NIB/akta pendirian yang tersimpan di kode maupun database — kalau dibutuhkan untuk dokumen Midtrans, itu murni dokumen fisik/administratif di luar codebase.
...
- **2026-07-03**: Dokumen dibuat. Domain fallback kode sudah diperbaiki ke `geeky.id` (blocker #5). Alamat & WhatsApp CS sudah dinamis di footer/contact (blocker #3/#4). Email publik sedang diseragamkan ke `support@geeky.id` (item #8, putaran ini).
