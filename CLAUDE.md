...
Apple's web presence is a masterclass in **reverent product photography framed by near-invisible UI**. Every page is a stack of edge-to-edge product "tiles" — alternating light and dark canvases, each centered on a hero headline, a one-line tagline, two tiny GeekyTech-red pill CTAs (`#EA5329`), and an impossibly crisp product render. Nothing competes with the product. Typography is confident but quiet; color is either pure white, an off-white parchment, or a near-black tile; interactive elements use the single GeekyTech accent red (`#EA5329`).

Density is unusually low even by contemporary SaaS standards. Each tile occupies roughly one viewport, and there is no decorative chrome — no borders, no gradients, no decorative frames, no shadows on headlines. Elevation appears only when a product image rests on a surface (a single soft `rgba(0, 0, 0, 0.22) 3px 5px 30px` drop for visual weight). The result is a catalog that feels more like a museum gallery: the wall disappears and the artifact takes over.

Store and shop surfaces retain the same chassis but switch modes. The product configurator (iPhone 17 Pro, accessories grid) introduces a tight grid of white utility cards at `{rounded.lg}` (18px) radius with a thin border, paired with a persistent thin sub-nav strip. The environment page leans darker and more editorial. Across all five surfaces the typographic system, spacing rhythm, and the single GeekyTech red accent are consistent — this is one design language expressed at different volumes.

**Key Characteristics:**
- Photography-first presentation; UI recedes so the product can speak.
- Alternating full-bleed tile sections: white/parchment ↔ near-black, with the color change itself acting as the section divider.
- Single GeekyTech red accent (`{colors.primary}` — #EA5329) carries every interactive element. No second brand color exists.
...
### Brand & Accent
- **Action Red / Brand** (`{colors.primary}` — #EA5329): GeekyTech's sole interactive accent. All text links, pill CTAs ("Learn more", "Buy"), and the focus-ring root. Prefer `transform: scale(0.95)` on press and a slightly darker hover (`#d44820`) over juggling many hex states.
- **Focus Red** (`{colors.primary-focus}` — #FF7A52): Slightly brighter than primary for keyboard focus outlines on buttons (`outline: 2px solid`) so the ring stays visible against `#EA5329` fills.
- **Accent on dark** (`{colors.primary-on-dark}` — #FFAD88): Lighter red-orange for in-copy links and callouts on **dark tiles** (`{colors.surface-tile-1}` etc.) where full-strength `#EA5329` reads too heavy.
...
| `{rounded.pill}` | 9999px | Primary GeekyTech-red pill CTAs, sub-nav buy button, configurator option chips, search input — signature pill shape |
...
**`button-primary`** — The signature primary action. Background `{colors.primary}` (GeekyTech #EA5329), text `{colors.on-primary}` in `{typography.body}` (SF Pro Text 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
- Active state: `{component.button-primary-active}` — `transform: scale(0.95)` (the system-wide micro-interaction).
- Focus state: `{component.button-primary-focus}` — 2px solid `{colors.primary-focus}` outline.

**`button-secondary-pill`** — Used as the second CTA when two merah pills appear together ("Learn more" / "Buy"). Background transparent, text `{colors.primary}`, 1px solid `{colors.primary}` border, rounded `{rounded.pill}`, padding 11px × 22px. Reads as a "ghost pill."
...
**`button-store-hero`** — A larger primary CTA used on store hero surfaces. Same GeekyTech red + on-primary text as `{component.button-primary}`, but with `{typography.button-large}` (18px / 300 — note the rare weight 300) and slightly more padding (14px × 28px). Used sparingly on the store landing.
...
**`text-link`** — Inline body links in `{colors.primary}` (#EA5329). Underlined or non-underlined per context.

**`text-link-on-dark`** — Inline body links on dark tiles in `{colors.primary-on-dark}` (#FFAD88) — `#EA5329` full strength can read muddy against `{colors.surface-tile-1}`; use the lighter token.
...
**`product-tile-dark`** — Full-bleed dark tile. Background `{colors.surface-tile-1}` (#272729), text `{colors.on-dark}`, rounded `{rounded.none}`, vertical padding `{spacing.section}` (80px). Same content stack as the light tile but with `{component.text-link-on-dark}` for inline copy and `{component.button-primary}` (GeekyTech red `#EA5329` on the dark surface). Used on the homepage product grid as the alternating dark band.
...
### Do
- Use `{colors.primary}` (GeekyTech #EA5329) for every interactive element — links, pill CTAs, focus signals — and nothing else. The single accent is non-negotiable.
...
- Reserve `{rounded.pill}` for the primary GeekyTech-red CTA and any other element that should read as an "action" (configurator chips, search input, sticky bar CTA).
...
### Don't
- Don't introduce a second accent color; every "click me" signal is `{colors.primary}` (GeekyTech #EA5329).
...
- Don't use `{colors.primary-on-dark}` (#FFAD88) on light surfaces — it's the dark-tile-only variant. `{colors.primary}` (#EA5329) is for light surfaces.
